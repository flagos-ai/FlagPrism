    ...
    def build_extension(self, ext: Extension) -> None:
        ...
        build_dir = Path(
            os.environ.get(
                ...
                str(Path(self.build_temp).resolve() / "profiler-component"),
            )
        ).resolve()
        ...
        core_build_dir_value = os.environ.get("FLAGTREE_BUILD_DIR", "")
        if not core_build_dir_value:
            raise RuntimeError(
                "building flagtree-profiler requires FLAGTREE_BUILD_DIR pointing "
                "to the FlagTree core CMake build tree"
            )
        core_build_dir = Path(core_build_dir_value).resolve()
        ...
        if not llvm_dir or not mlir_dir:
            raise RuntimeError(
                "building flagtree-profiler requires LLVM_DIR and MLIR_DIR, or LLVM_SYSPATH"
            )

        json_include = os.environ.get("JSON_INCLUDE_DIR") or os.environ.get("JSON_SYSPATH")
        if not json_include:
            raise RuntimeError(
                "building flagtree-profiler requires JSON_INCLUDE_DIR pointing to nlohmann/json.hpp"
            )
        cupti_include = os.environ.get(
            "TRITON_CUPTI_INCLUDE_PATH",
            str(source_dir / "third_party/nvidia/backend/include"),
        )
        roctracer_include = os.environ.get(
            "TRITON_ROCTRACER_INCLUDE_PATH",
            str(source_dir / "third_party/amd/backend/include"),
        )
        cmake = shutil.which("cmake")
        if cmake is None:
            raise RuntimeError("building flagtree-profiler requires CMake >= 3.20")
        ...
        configure = [
            ...
            f"-DFLAGTREE_BUILD_DIR={core_build_dir}",
            f"-DFLAGTREE_LIBTRITON={libtriton_path}",
            f"-DJSON_INCLUDE_DIR={json_include}",
            f"-DCUPTI_INCLUDE_DIR={cupti_include}",
            f"-DROCTRACER_INCLUDE_DIR={roctracer_include}",
        ...
        subprocess.check_call([cmake, "--build", str(build_dir), "--target", "proton"])


build_native = os.environ.get("FLAGTREE_COMPONENT_BUILD_NATIVE", "1") != "0"
setup(
    ext_modules=[CMakeExtension("flagtree_profiler.libproton")] if build_native else [],
