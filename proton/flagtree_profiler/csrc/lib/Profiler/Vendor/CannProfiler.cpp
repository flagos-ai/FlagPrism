  ...
  for (const auto &root : uniqueRoots) {
    ...
    while (it != end) {
      ...
      if (it->is_regular_file(entryEc) && !entryEc) {
        auto fileName = toLower(it->path().filename().string());
...
void CannProfiler::doSetMode(const std::vector<std::string> &modeAndOptions) {
  ...
  deviceId = 0;
  outputPath = getEnvOrDefault("PROTON_CANN_PROFILE_OUTPUT", "./proton_cann_profile");
  memFreqHz = "15";
  ...
  if (!hostTimingFallbackEnabled) {
    auto envFlag = toLower(getEnvOrDefault("PROTON_CANN_RUNTIME_HOST_FALLBACK", "0"));
