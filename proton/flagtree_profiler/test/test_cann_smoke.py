...
def test_native_instrumentation_preserves_mode_and_default_hook(monkeypatch):
    ...
    monkeypatch.setattr(proton_profile, "libproton", runtime)
    monkeypatch.setattr(
        proton_profile.triton.runtime.driver,
        "_active",
        SimpleNamespace(
            get_current_target=lambda: SimpleNamespace(backend="cuda"),
        ),
    )
    monkeypatch.setattr(proton_profile, "set_profiling_on", lambda: None)
    monkeypatch.setattr(proton_profile, "is_command_line", lambda: False)
    monkeypatch.setattr(
        proton_profile,
        "InstrumentationHook",
    ...
    monkeypatch.setattr(
        proton_profile.HookManager,
        "register",
        staticmethod(lambda hook, session: calls.setdefault("hook", (hook, session))),
    )
    proton_profile._active_sessions.clear()
    try:
        session = proton_profile.start(
            "/tmp/proton_instrumentation_compat",
...
def test_finalize_preserves_data_specific_default_format(monkeypatch):
    ...
    monkeypatch.setattr(proton_profile, "libproton", runtime)
    monkeypatch.setattr(proton_profile, "set_profiling_off", lambda: None)
    monkeypatch.setattr(proton_profile, "is_command_line", lambda: False)
    monkeypatch.setattr(
        proton_profile.HookManager,
        "unregister",
    ...
    proton_profile._active_sessions.clear()
    proton_profile._active_sessions[72] = {"name": "trace"}
    proton_profile.finalize(72)
...
def test_cann_real_direct_imports_mstx_timeline(real_cann_direct_run):
    ...
    assert any(
        ...
        == "proton_cann_triton::triton_vector_add_fp32"
        for assoc in vendor_json.get("associations", [])
    )
