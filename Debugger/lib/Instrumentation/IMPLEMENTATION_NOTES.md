...
**负责人**：颜臻
**模块路径**：`third_party/FlagPrism/Debugger/lib/Instrumentation/` + `third_party/FlagPrism/Debugger/include/Debugger/Instrumentation/`
...
| 文件 | 性质 | 说明 |
|------|------|------|
| `third_party/FlagPrism/Debugger/include/Debugger/Instrumentation/Passes.h` | 接口 | Pass 入口声明 |
| `third_party/FlagPrism/Debugger/include/Debugger/Instrumentation/Collectors.h` | 接口 | collector 规格 + `SummaryStats` + 计算函数声明 |
| `third_party/FlagPrism/Debugger/include/Debugger/Instrumentation/RecordBuilder.h` | 接口 | host-side record 构造辅助函数声明 |
| `third_party/FlagPrism/Debugger/include/Debugger/Instrumentation/Writer.h` | 接口 | ring buffer 操作 + `RecordSink` 抽象类 + 工厂函数 |
| `third_party/FlagPrism/Debugger/lib/Instrumentation/Passes.cpp` | 实现 | `InsertInstrumentationPass` |
| `third_party/FlagPrism/Debugger/lib/Instrumentation/Collectors.cpp` | 实现 | collector 规格表 + host-side 统计计算 |
| `third_party/FlagPrism/Debugger/lib/Instrumentation/RecordBuilder.cpp` | 实现 | record 构造辅助函数 |
| `third_party/FlagPrism/Debugger/lib/Instrumentation/Writer.cpp` | 实现 | ring buffer 操作 + `LinearAppendSink` + `RingBufferSink` |
| `third_party/FlagPrism/Debugger/test/unittest/InstrumentationTest.cpp` | 测试 | 14 个单元测试 |
...
**实现文件**：`third_party/FlagPrism/Debugger/lib/Instrumentation/Writer.cpp`，匿名 namespace 内的 `LinearAppendSink` 类。
...
测试文件：`third_party/FlagPrism/Debugger/test/unittest/InstrumentationTest.cpp`
