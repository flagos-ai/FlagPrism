...
from flaggems_debug_batch import (  # noqa: E402
    DEFAULT_FLAGGEMS_ROOT, DEFAULT_PYTHON, InstrumentationStats, build_env,
    classify_status, copy_flaggems_source, create_bootstrap,
    first_error_from_logs, instrument_flaggems_tree, load_operator_inventory,
    no_cpu_ops, now_stamp, run_phase_for_op, shell_command, write_text,
)
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        baseline.append(
            BaselineItem(
                ...
                baseline_debug_txt_count=int(
                    run_info.get("debug_txt_count") or 0),
                baseline_debug_json_count=int(
                    run_info.get("debug_json_count") or 0),
                baseline_duration_sec=(float(run_info["duration_sec"])
                                       if run_info.get("duration_sec")
...
def select_items(items: list[BaselineItem],
                 args: argparse.Namespace) -> list[BaselineItem]:
    selected = items
    if args.kinds:
    ...
    if args.start_index:
        selected = [
            item for item in selected if item.index >= args.start_index
        ]
    if args.max_items:
...
def wait_for_process_with_reports(
    ...
    *,
    ...
    while True:
        ...
        elif (report_timeout is not None and last_report_time is not None
              and now - last_report_time > report_timeout):
            kill_process_group(proc)
            return None, True, "report_timeout"
        time.sleep(poll_interval)


def classify_comparison(item: BaselineItem,
                        result: RegressionResult) -> tuple[str, list[str]]:
    warnings: list[str] = []
    if result.status != "passed":
        return "regressed", warnings
    if result.debug_txt_count <= 0 or result.debug_json_count <= 0:
        return "regressed", warnings
    if (item.baseline_debug_txt_count > 0
            and result.debug_txt_count < item.baseline_debug_txt_count):
        warnings.append(
            f"txt reports decreased {item.baseline_debug_txt_count}->{result.debug_txt_count}"
        )
    if (item.baseline_debug_json_count > 0
            and result.debug_json_count < item.baseline_debug_json_count):
        warnings.append(
            f"json reports decreased {item.baseline_debug_json_count}->{result.debug_json_count}"
        )
    if item.baseline_duration_sec and result.duration_sec > 2 * item.baseline_duration_sec:
        warnings.append(
            f"duration >2x baseline {item.baseline_duration_sec:.1f}s->{result.duration_sec:.1f}s"
        )
...
def run_saved_script_case(
    ...
    if status == "missing_debug_report":
        first_error = (
            "case exited successfully but debugger report is missing or incomplete: "
...
def run_pytest_node_item(
    ...
    env_extra = {
        ...
        str(args.addr_level),
        "FLAGTREE_DEBUGGER_NODE_EXPORT_RAW":
        "1" if args.export_raw_records else "0",
    }
    return run_saved_script_case(
        item,
        source_script,
...
def run_legacy_marker_item(
    ...
    result = make_base_result(
        ...
        if status.status in {"timeout", "partial_timeout"} else "",
    )
    write_json(legacy_dir / "status.json", asdict(result))
    return result


def write_markdown_report(run_dir: Path, baseline: list[BaselineItem],
                          results: list[RegressionResult]) -> None:
    by_status: dict[str, int] = {}
    by_comparison: dict[str, int] = {}
    for result in results:
        by_status[result.status] = by_status.get(result.status, 0) + 1
        by_comparison[result.comparison_status] = by_comparison.get(
            result.comparison_status, 0) + 1
    ...
    if not regressions:
        ...
        for result in regressions:
            lines.append(
                f"- {result.index:03d} {result.op} [{result.kind}] "
    ...
    warnings = [
        r for r in results if r.comparison_status == "passed_with_warning"
    ]
    if not warnings:
        lines.append("None.")
    else:
...
def write_progress(run_dir: Path, baseline: list[BaselineItem],
                   results: list[RegressionResult]) -> None:
    write_json(run_dir / "comparison.json",
               [asdict(result) for result in results])
    status_counts: dict[str, int] = {}
    comparison_counts: dict[str, int] = {}
    for result in results:
        status_counts[result.status] = status_counts.get(result.status, 0) + 1
        comparison_counts[result.comparison_status] = (
            comparison_counts.get(result.comparison_status, 0) + 1)
    ...
    write_json(
        ...
        [
            asdict(r)
            for r in results if r.comparison_status == "passed_with_warning"
        ],
    )
...
def write_manifest(
    ...
    manifest = {
        ...
        "case_total_timeout":
        args.case_total_timeout,
        "first_report_timeout":
        args.first_report_timeout,
        "report_timeout":
        args.report_timeout,
        "instrument_pointwise_generated":
        args.instrument_pointwise_generated,
        "normalize_ext_launch_ids":
        args.normalize_ext_launch_ids,
        "prune_item_caches":
        args.prune_item_caches,
        "shared_compiler_cache":
        args.shared_compiler_cache,
        "shared_cache_dir":
        (str(args.shared_cache_dir) if args.shared_cache_dir else None),
        "instrumentation":
        asdict(stats),
    }
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    def add_bool_argument(name: str,
                          *,
                          default: bool,
                          help_text: str = "") -> None:
        dest = name.replace("-", "_")
    ...
    parser = argparse.ArgumentParser(
        description=
        "Replay FlagGems debugger samples and compare against samples/index.json."
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Index file under --samples-root. Use index.json to replay stable "
    ...
    parser.add_argument(
        "--kinds",
        help=
        "comma-separated baseline kinds: direct_case,pytest_node,legacy_marker",
    )
    ...
    parser.add_argument("--timeout",
                        type=int,
                        default=1800,
                        help="Legacy marker pytest timeout.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        ("Delete each completed item's rebuildable Triton and FlagGems caches "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        ("Reuse a run-local Triton/FlagGems compiler cache across sequential "
    ...
    add_bool_argument(
        ...
        default=True,
        help_text=
        "Stop the replay after an NPU/device error so later cases are not run on a poisoned device context.",
    )
...
def main(argv: list[str]) -> int:
    ...
    args.shared_cache_dir = (run_dir / "shared_compiler_cache"
                             if args.shared_compiler_cache else None)
    create_bootstrap(bootstrap_dir)

    write_json(run_dir / "baseline_all.json",
               [asdict(item) for item in baseline])
    write_json(run_dir / "baseline_selected.json",
    ...
    if args.dry_run:
        write_manifest(run_dir, args, worktree, selected,
    ...
    for position, item in enumerate(selected, start=1):
        ...
        try:
            if item.kind == "direct_case":
                result = run_direct_item(item, worktree, run_dir,
                                         bootstrap_dir, args)
            elif item.kind == "pytest_node":
                result = run_pytest_node_item(item, worktree, run_dir,
                                              bootstrap_dir, args)
            elif item.kind == "legacy_marker":
                result = run_legacy_marker_item(item, worktree, run_dir,
        ...
        if args.prune_item_caches:
            ...
            if removed:
                print(
                    f"[CLEAN] removed {len(removed)} transient item cache(s)")
        ...
        if args.stop_on_device_error and result.status == "device_error":
            print(
                "[STOP] device_error encountered; stopping before later cases "
    ...
    regressions = [
        result for result in results if result.comparison_status == "regressed"
    ]
    print(
        f"[DONE] regressions={len(regressions)} comparison={run_dir / 'comparison.md'}"
    )
