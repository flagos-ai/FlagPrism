        ...
        baseline.append(
            BaselineItem(
                ...
                baseline_duration_sec=(
                    float(run_info["duration_sec"])
                    if run_info.get("duration_sec") is not None
                    else None
                ),
...
def classify_comparison(item: BaselineItem, result: RegressionResult) -> tuple[str, list[str]]:
    ...
    if result.debug_txt_count <= 0 or result.debug_json_count <= 0:
        return "regressed", warnings
    if (
        item.baseline_debug_txt_count > 0
    ...
    if (
        item.baseline_debug_json_count > 0
...
def main(argv: list[str]) -> int:
    ...
    args.shared_cache_dir = (
        run_dir / "shared_compiler_cache" if args.shared_compiler_cache else None
    )
    ...
    for position, item in enumerate(selected, start=1):
        ...
        print(
            f"[RESULT] {item.op}: {result.comparison_status} "
