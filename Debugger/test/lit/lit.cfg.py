...
config.test_exec_root = os.path.join(config.triton_obj_root, "third_party",
                                     "Debugger", "test", "lit")
...
if filecheck and os.path.isfile(filecheck):
    ...
    fallback = os.path.join(os.path.dirname(__file__), "Inputs",
                            "filecheck.py")
...
config.substitutions.append(
    ("not ", f'"{config.python_executable}" "{not_fallback}" '))
