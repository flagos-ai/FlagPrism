...
def run_probe() -> dict:
    ...
    # while retaining the wrapper ABI generated from the original metadata.
    launcher.metadata = SimpleNamespace(debug_enabled=False)
    log_lines.append(f"stream: {int(stream)}")
    log_lines.append(f"debug_hidden_arg: {debug_ctrl_ptr}")
