...
这个例子用于演示 Profiler scope 如何把多个 Triton kernel 组织成一个简单神经
网络调用树。它比 GPT 示例小，适合先解释多算子 profile 的基本结构。

运行：

    cd /workspace/FlagTree
    python3 third_party/FlagPrism/Profiler/examples/tiny_mlp_profile.py
...
查看结果：
    ...
    flagtree-profiler-viewer -m time/us,cann.task_duration_us,cann.aicore_time_us,cann.bandwidth_gb_s,cann.memory_access_bytes \
      /tmp/flagtree_profiler_examples/tiny_mlp/profile.hatchet
