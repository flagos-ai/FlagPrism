...
class scope:
    ...
    Usage:
        context manager:
        ```python
        with profiler.scope("test0", {metric_name: metric_value}):
            foo[1,](x, y)
        ```

        decorator:
        ```python
        @profiler.scope("test0", {metric_name: metric_value})
        def foo(x, y):
            ...
        ```
    ...
    def _enter_scope(self):
        if not get_profiling_on():
            return
        self.id = profiler_native.record_scope()
        profiler_native.enter_scope(self.id, self.name)
        if self.metrics:
            profiler_native.add_metrics(self.id, self.metrics)

    def _exit_scope(self):
        if not get_profiling_on() or self.id is None:
            return
        profiler_native.exit_scope(self.id, self.name)
...
def enter_scope(name: str, *, metrics: Optional[dict[str, MetricValueType]] = None) -> Optional[int]:
    ...
    profiler_native.enter_scope(id, name)
    if metrics:
        profiler_native.add_metrics(id, metrics)
    return id
...
def exit_scope(name: Optional[str] = None, *, metrics: Optional[dict[str, MetricValueType]] = None) -> Optional[int]:
    ...
    if name and name != popped_name:
        ...
        name = popped_name
    profiler_native.exit_scope(id, name)
    if metrics:
        profiler_native.add_metrics(id, metrics)
    return id
