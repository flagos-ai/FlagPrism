...
def _uses_default_ir_triton_hook(backend: Optional[str],
    ...
    return (hook == "triton" and _is_cann_backend(backend)
...
def start(
    ...
    *,
    ...
    use_triton_hook = hook == "triton" or (hook == "instrumentation"
                                           and _is_cann_backend(backend))
    ir_default_for_triton = _uses_default_ir_triton_hook(backend, hook)
    use_instrumentation_hook = (_is_cann_backend(backend)
                                and (hook == "instrumentation"
    ...
    effective_mode = (_mode_with_ir_triton_overrides(mode) if
