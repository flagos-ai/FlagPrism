#!/usr/bin/env python3
"""Run a small real NPU workload under FlagTree Profiler CANN profiling.

This script is intended to be launched by external msprof, for example:

    msprof --msproftx=on --output=/tmp/flagtree_profiler_cann_real/msprof \
      python third_party/FlagPrism/Profiler/scripts/cann_real_npu_workload.py \
        --name /tmp/flagtree_profiler_cann_real/profile_run \
        --vendor-output /tmp/flagtree_profiler_cann_real/msprof

The process writes Profiler artifacts during profiler.finalize(). External msprof
...
def main() -> int:
    ...
    scope_name = "flagtree_profiler_cann_real_triton_vector_add"
