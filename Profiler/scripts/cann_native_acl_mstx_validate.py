...
exported CSV files are post-imported through the Profiler CANN importer.
"""
...
int main(int argc, char **argv) {
  ...
  auto domain = mstxDomainCreateA("flagtree_profiler");
  uint64_t range_id = 0;
  if (domain) {
    range_id = mstxDomainRangeStartA(domain, "flagtree_profiler_cann_native_acl_mstx_probe", stream);
  } else {
    range_id = mstxRangeStartA("flagtree_profiler_cann_native_acl_mstx_probe", stream);
  }
