#!/usr/bin/env python3
"""Profile Triton-developed operators with FlagTree Profiler CANN.

Default mode is a driver:

    python third_party/FlagPrism/Profiler/scripts/cann_operator_profile_suite.py \
      --out /tmp/flagtree_profiler_cann_triton_operator_suite --clean
...
Profiler artifacts, and writes summary.json.
"""
...
@triton.jit
def _vector_fma_kernel(x_ptr, y_ptr, z_ptr, out_ptr, n_elements,
                       BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(axis=0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n_elements
...
@triton.jit
def _relu_kernel(x_ptr, bias_ptr, out_ptr, n_elements,
                 BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(axis=0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n_elements
...
@triton.jit
def _row_sum_kernel(x_ptr, out_ptr, n_cols: tl.constexpr,
                    BLOCK_SIZE: tl.constexpr):
...
@triton.jit
def _row_max_kernel(x_ptr, out_ptr, n_cols: tl.constexpr,
                    BLOCK_SIZE: tl.constexpr):
    row = tl.program_id(axis=0)
    cols = tl.arange(0, BLOCK_SIZE)
    mask = cols < n_cols
...
@triton.jit
def _transpose_kernel(x_ptr, out_ptr, M: tl.constexpr, N: tl.constexpr,
...
@triton.jit
def _masked_tail_kernel(x_ptr, y_ptr, out_ptr, n_elements,
                        BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(axis=0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n_elements
...
def _load_torch_npu():
    try:
        ...
        raise RuntimeError(
            "PyTorch is required for the Triton operator suite.") from exc
...
def _triton_cases(torch, device) -> list[TritonCase]:
    ...
    def vector_add_fp32():
        ...
        return lambda: (_vector_add_kernel[_grid_1d(n, block)]
                        (x, y, out, n, BLOCK_SIZE=block), out)[1]
    ...
    def vector_fma_fp32():
        ...
        out = torch.empty_like(x)
        return lambda: (_vector_fma_kernel[_grid_1d(n, block)]
                        (x, y, z, out, n, BLOCK_SIZE=block), out)[1]
    ...
    def relu_fp16():
        ...
        out = torch.empty_like(x)
        return lambda: (_relu_kernel[_grid_1d(n, block)]
                        (x, bias, out, n, BLOCK_SIZE=block), out)[1]
    ...
    def exp_fp32():
        ...
        x = torch.randn((n, ), device=device, dtype=torch.float32)
        out = torch.empty_like(x)
        return lambda: (_exp_kernel[_grid_1d(n, block)]
                        (x, out, n, BLOCK_SIZE=block), out)[1]
    ...
    def copy_fp16():
        ...
        x = torch.randn((n, ), device=device, dtype=torch.float16)
        out = torch.empty_like(x)
        return lambda: (_copy_kernel[_grid_1d(n, block)]
                        (x, out, n, BLOCK_SIZE=block), out)[1]
    ...
    def cast_fp32_to_fp16():
        ...
        return lambda: (_cast_kernel[_grid_1d(n, block)]
                        (x, out, n, BLOCK_SIZE=block), out)[1]
    ...
    def row_sum_fp32():
        ...
        return lambda: (_row_sum_kernel[(rows, )]
                        (x, out, cols, BLOCK_SIZE=block), out)[1]
    ...
    def row_max_fp32():
        ...
        x = torch.randn((rows, cols), device=device, dtype=torch.float32)
        out = torch.empty((rows, ), device=device, dtype=torch.float32)
        return lambda: (_row_max_kernel[(rows, )]
                        (x, out, cols, BLOCK_SIZE=block), out)[1]
    ...
    def softmax_fp32():
        ...
        x = torch.randn((rows, cols), device=device, dtype=torch.float32)
        out = torch.empty_like(x)
        return lambda: (_softmax_kernel[(rows, )]
                        (x, out, cols, BLOCK_SIZE=block), out)[1]
    ...
    def transpose_fp16():
        ...
        return lambda: (_transpose_kernel[grid]
                        (x, out, m, n, BLOCK_M=bm, BLOCK_N=bn), out)[1]
    ...
    def matmul_fp16():
        ...
        grid = (triton.cdiv(m, bm) * triton.cdiv(n, bn), )
        return lambda: (_matmul_kernel[grid](
            a, b, c, m, n, k, BLOCK_M=bm, BLOCK_N=bn, BLOCK_K=bk), c)[1]
    ...
    def masked_tail_fp32():
        ...
        y = torch.randn((n, ), device=device, dtype=torch.float32)
        out = torch.empty_like(x)
        return lambda: (_masked_tail_kernel[_grid_1d(n, block)]
                        (x, y, out, n, BLOCK_SIZE=block), out)[1]

    return [
        TritonCase("triton_vector_add_fp32", "elementwise",
                   "tl.load + add + tl.store, fp32", vector_add_fp32),
        TritonCase("triton_vector_fma_fp32", "elementwise",
                   "tl.load + multiply-add + tl.store", vector_fma_fp32),
        TritonCase("triton_relu_fp16", "activation",
                   "tl.where relu-style activation", relu_fp16),
        TritonCase("triton_exp_fp32", "math", "tl.exp vector kernel",
                   exp_fp32),
        TritonCase("triton_copy_fp16", "memory", "contiguous load/store copy",
                   copy_fp16),
        TritonCase("triton_cast_fp32_to_fp16", "cast",
                   "fp32 load converted to fp16 store", cast_fp32_to_fp16),
        TritonCase("triton_row_sum_fp32", "reduction", "row-wise tl.sum",
                   row_sum_fp32),
        TritonCase("triton_row_max_fp32", "reduction", "row-wise tl.max",
                   row_max_fp32),
        TritonCase("triton_softmax_fp32", "normalization",
                   "fused row-wise softmax", softmax_fp32),
        TritonCase("triton_transpose_fp16", "layout", "2D tiled transpose",
                   transpose_fp16),
        TritonCase("triton_matmul_fp16", "dense_compute",
                   "tl.dot matrix multiplication", matmul_fp16),
        TritonCase("triton_masked_tail_fp32", "masking",
                   "non-power-of-two masked tail kernel", masked_tail_fp32),
    ]
...
def _make_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out", default="/tmp/flagtree_profiler_cann_triton_operator_suite")
    ...
    parser.add_argument("--external-msprof",
                        action="store_true",
                        help=argparse.SUPPRESS)
    return parser
...
def _run_workload(args: argparse.Namespace) -> int:
    ...
    for name in selected_names:
        ...
        scope_name = f"flagtree_profiler_cann_triton::{case.name}"
        try:
            op = case.make()
    ...
    try:
        for case, scope_name, op, baseline_elapsed_s in prepared:
            try:
                ...
                try:
                    profiled_elapsed_s, out = _measure_op(
                        torch, op, args.iters)
                finally:
    ...
    summary = {
        "ran_triton_kernel":
        bool(results) and any(r["status"] == "ok" for r in results),
        "device":
        str(device),
        "operator_count":
        len(selected_names),
        ...
        failures,
        "timing":
        _summarize_timing(results),
        "base":
        str(base),
        "vendor_json":
        str(base.with_suffix(".vendor.json")),
        "workload_summary_json":
        str(base.with_suffix(".workload_summary.json")),
    }
    base.with_suffix(".workload_summary.json").write_text(
...
def _run_baseline(args: argparse.Namespace) -> int:
    ...
    for name in selected_names:
        ...
        try:
            ...
            results.append({
                ...
                baseline_elapsed_s,
                "baseline_per_iter_s":
                baseline_elapsed_s / args.iters if args.iters else None,
            })
        except Exception as exc:
            failures.append({
                "name": case.name,
    ...
    summary = {
        ...
        str(base.with_suffix(".baseline_summary.json")),
    }
    base.with_suffix(".baseline_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True))
    print(json.dumps(summary, indent=2, sort_keys=True))
    if failures and not args.allow_op_failures:
        return 4
...
def _summarize_post_import(out: pathlib.Path,
                           post_import_base: pathlib.Path) -> dict:
    ...
    op_types = Counter(
        ...
        and assoc.get("metrics", {}).get("op_type"))
    triton_ranges = [
        msg for msg in mstx_messages
        if msg.startswith("flagtree_profiler_cann_triton::")
    ]
...
def _run_driver(args: argparse.Namespace) -> int:
    ...
    baseline_summary = (json.loads(baseline_summary_path.read_text())
                        if baseline_summary_path.exists() else {})
    baseline_by_name = {
