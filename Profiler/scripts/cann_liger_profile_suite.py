#!/usr/bin/env python3
"""Profile LinkedIn Liger-Kernel operators with FlagTree Profiler CANN.

This is the public-library full evaluation companion to
``cann_operator_profile_suite.py``. It intentionally uses the Profiler's direct
start/finalize flow: no user-visible external ``msprof python ...`` wrapper is
required.

Example:

    python third_party/FlagPrism/Profiler/scripts/cann_liger_profile_suite.py \
      --out /tmp/flagtree_profiler_cann_liger_full --clean
"""
...
def main() -> int:
    ...
    try:
        for case, op, baseline_elapsed_s in prepared:
            scope_name = f"flagtree_profiler_cann_liger::{case.name}"
            try:
