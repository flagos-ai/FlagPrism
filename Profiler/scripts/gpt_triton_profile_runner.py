    ...
    with profiler.scope("prefill"):
        ...
        with profiler.scope("self-attn"):
            ...
            with profiler.scope("qkv_proj"):
                q, k, v = kernels.matmul_and_split_qkv(
                    x, weights["qkv_w"], weights["qkv_b"], heads
                )
            ...
            with profiler.scope("o_proj"):
                x = kernels.fused_ffn(
                    x, weights["proj_w"], bias=weights["proj_b"], residual=residual
                )
        ...
        with profiler.scope("mlp"):
            ...
            with profiler.scope("gate_up_proj"):
                x = kernels.fused_ffn(
                    x, weights["ff1_w"], bias=weights["ff1_b"], add_gelu=True
                )
            with profiler.scope("down_proj"):
                x = kernels.fused_ffn(
                    x, weights["ff2_w"], bias=weights["ff2_b"], residual=residual
                )
