...
def _load_gpt_triton_kernels():
    ...
    if not (source / "kernels.py").exists():
        raise RuntimeError(
            f"gpt-triton source not found at {source}. Clone "
...
def _make_weights(torch, device, *, vocab: int, seq_len: int, hidden: int,
                  heads: int):
    del heads
    ...
    return {
        ...
        "ff1_w": torch.randn((hidden, intermediate),
                             device=device,
                             dtype=dtype),
        "ff1_b": torch.randn((intermediate, ), device=device, dtype=dtype),
        "ff2_w": torch.randn((intermediate, hidden),
                             device=device,
                             dtype=dtype),
...
def _gpt_block(kernels, token_ids, weights, *, heads: int):
    with profiler.scope("prefill"):
        with profiler.scope("embeddings"):
            x = kernels.fused_embeddings(token_ids, weights["wte"],
                                         weights["wpe"])

        with profiler.scope("self-attn"):
            residual = x
            with profiler.scope("attn_ln"):
                x = kernels.fused_layer_norm(x, weights["ln1_w"],
                                             weights["ln1_b"])
            with profiler.scope("qkv_proj"):
                q, k, v = kernels.matmul_and_split_qkv(x, weights["qkv_w"],
                                                       weights["qkv_b"], heads)
            with profiler.scope("attn"):
                x = kernels.flash_attention_v1(q, k, v)
            x = x.transpose(1, 2).contiguous().view(residual.shape)
            with profiler.scope("o_proj"):
                x = kernels.fused_ffn(x,
                                      weights["proj_w"],
                                      bias=weights["proj_b"],
                                      residual=residual)
        ...
        with profiler.scope("mlp"):
            residual = x
            with profiler.scope("mlp_ln"):
                x = kernels.fused_layer_norm(x, weights["ln2_w"],
                                             weights["ln2_b"])
            with profiler.scope("gate_up_proj"):
                x = kernels.fused_ffn(x,
                                      weights["ff1_w"],
                                      bias=weights["ff1_b"],
                                      add_gelu=True)
            with profiler.scope("down_proj"):
                x = kernels.fused_ffn(x,
                                      weights["ff2_w"],
                                      bias=weights["ff2_b"],
                                      residual=residual)

        with profiler.scope("final_ln"):
            return kernels.fused_layer_norm(x, weights["lnf_w"],
                                            weights["lnf_b"])
...
def main() -> int:
    ...
    out_dir = pathlib.Path(
        os.environ.get("GPT_TRITON_PROFILE_OUT",
                       "/tmp/flagtree_profiler_gpt_triton"))
    if out_dir.exists():
