#!/usr/bin/env python3
"""Profile FlagGems benchmark operators with FlagTree Profiler CANN.
...
and once inside ``profiler.start(..., backend="cann", hook="triton")``. It keeps
the FlagGems checkout untouched and imports it from source.

Example:

    python third_party/FlagPrism/Profiler/scripts/cann_flaggems_profile_suite.py \
...
def _worker_main(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.phase == "profiled":
            ...
            scope_name = f"flagtree_profiler_cann_flaggems::{args.case_name}"
