...
def test_context(source: str, tmp_path: pathlib.Path):
    ...
    session_id = profiler_native.start(str(temp_file.with_suffix("")), source, "tree", _select_backend(), "")
...
def test_session(tmp_path: pathlib.Path):
    ...
    session_id = profiler_native.start(str(temp_file.with_suffix("")), "shadow", "tree", _select_backend())
...
def test_add_metrics(tmp_path: pathlib.Path):
    ...
    profiler_native.start(str(temp_file.with_suffix("")), "shadow", "tree", _select_backend())
    id1 = profiler_native.record_scope()
    profiler_native.enter_scope(id1, "one")
