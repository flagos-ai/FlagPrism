...
file_path = __file__
triton_example_file = file_path.replace("test_viewer.py",
                                        "examples/triton.json")
...
leaf_example_file = file_path.replace("test_viewer.py",
                                      "examples/leaf_nodes.json")


def test_help():
    # Only check if the viewer can be invoked
    subprocess.check_call(["flagtree-profiler-viewer", "-h"],
                          stdout=subprocess.DEVNULL)


def test_exclusive_metrics():
    gf, inclusive_metrics, exclusive_metrics, device_info = read(
        triton_example_file)
    metrics = ["cpu_time/ns"]
...
def test_sort():
    ...
    expected = [
        "ROOT", "kernel_1_1_1", "kernel_3_1_1", "kernel_3_2_2", "kernel_1_2_2"
    ]
...
@pytest.mark.parametrize(
    "option", ["full", "file_function_line", "function_line", "file_function"])
...
def test_min_time_flops():
    ...
    # sm89
    np.testing.assert_allclose(ret[device0_idx].to_numpy(), [[0.000025]],
                               atol=1e-5)
    # sm90
    np.testing.assert_allclose(ret[device1_idx].to_numpy(), [[0.00005]],
                               atol=1e-5)
    # sm100
    np.testing.assert_allclose(ret[device2_idx].to_numpy(), [[0.000025]],
                               atol=1e-5)
    ...
    device2_idx = gf.dataframe["device_id"] == "2"
    # CDNA2
    np.testing.assert_allclose(ret[device0_idx].to_numpy(), [[0.000055]],
                               atol=1e-5)
    # CDNA3
    np.testing.assert_allclose(ret[device1_idx].to_numpy(), [[0.000038]],
                               atol=1e-5)
    # CDNA4
    np.testing.assert_allclose(ret[device2_idx].to_numpy(), [[0.000217]],
                               atol=1e-5)
...
def test_min_time_bytes():
    ...
    device1_idx = gf.dataframe["device_id"] == "1"
    # sm89
    np.testing.assert_allclose(ret[device0_idx].to_numpy(), [[9.91969e-06]],
                               atol=1e-6)
    # sm90
    np.testing.assert_allclose(ret[device1_idx].to_numpy(), [[2.48584e-05]],
                               atol=1e-6)
    gf, _, _, device_info = read(hip_example_file)
    ret = get_min_time_bytes(gf.dataframe, device_info)
    device0_idx = gf.dataframe["device_id"] == "0"
    ...
    device2_idx = gf.dataframe["device_id"] == "2"
    # CDNA2
    np.testing.assert_allclose(ret[device0_idx].to_numpy(), [[3.125e-06]],
                               atol=1e-6)
    # CDNA3
    np.testing.assert_allclose(ret[device1_idx].to_numpy(), [[1.93378e-05]],
                               atol=1e-6)
    # CDNA4
    np.testing.assert_allclose(ret[device2_idx].to_numpy(), [[0.000125]],
                               atol=1e-6)
...
def derivation_metrics_test(metrics,
                            expected_data,
                            sample_file,
                            rtol=1e-7,
                            atol=1e-6):
    ...
    derived_metrics = derive_metrics(gf, metrics, inclusive_metrics,
                                     exclusive_metrics, device_info)
    for derived_metric in derived_metrics:
        np.testing.assert_allclose(gf.dataframe[derived_metric].to_numpy(),
                                   expected_data[derived_metric],
                                   rtol=rtol,
                                   atol=atol)
...
def test_avg_time_derivation():
    derivation_metrics_test(
        ...
        expected_data={
            "avg_time/s (inc)": [0.0000512, 0.0000205, 0.000205, 0.000205],
            "avg_time/ms (inc)": [0.0512, 0.02048, 0.2048, 0.2048],
            "avg_time/us (inc)": [51.2, 20.48, 204.8, 204.8],
            "avg_time/ns (inc)": [51200.0, 20480.0, 204800.0, 204800.0]
        },
        sample_file=cuda_example_file)


def test_util():
    derivation_metrics_test(metrics=["util"],
                            expected_data={
...
def test_bytes_derivation():
    derivation_metrics_test(
        ...
        expected_data={
            "byte/s (inc)":
            [1.953125e+11, 4.88281250e+11, 4.88281250e+10, 4.88281250e+10],
...
def test_flops_derivation():
    derivation_metrics_test(
        ...
        expected_data={
            "flop8/s (inc)":
            [3.417969e+14, 4.88281250e+14, 4.88281250e+13, 4.88281250e+14],
            "gflop8/s (inc)": [341796.875, 488281.25, 48828.125, 488281.25],
