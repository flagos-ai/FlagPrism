...
@pytest.mark.parametrize(
    "mode",
    [
        profiler.mode.Default(),
        profiler.mode.Default(metric_type="cycle"),
        profiler.mode.Default(metric_type="cycle", buffer_size=4096),
        profiler.mode.MMA(),
    ],
)
...
def test_jit(tmp_path):
    ...
    profiler.start(str(temp_file.with_suffix("")), backend="instrumentation")
...
def test_record(method, tmp_path: pathlib.Path):
    ...
    def instrumentation(file_path):
        ...
        try:
            ...
            profiler.hooks.InstrumentationHook.enable_host_buffer = False
            profiler.finalize()
