# FlagTree Profiler CANN FlagGems Suite
...
python3 third_party/FlagPrism/Profiler/scripts/cann_profile_test_suite.py \
  --out /tmp/flagtree_profiler_cann_flaggems_full \
  --clean \
  --with-flaggems \
  --flaggems-all
```
...
```bash
python3 third_party/FlagPrism/Profiler/scripts/cann_profile_test_suite.py \
  --out /tmp/flagtree_profiler_cann_flaggems_add \
  --clean \
  --with-flaggems \
  --flaggems-op add
```
...
```bash
python3 third_party/FlagPrism/Profiler/scripts/cann_profile_test_suite.py \
  --out /tmp/flagtree_profiler_cann_flaggems_default \
  --clean \
  --with-flaggems
```
...
```bash
python3 third_party/FlagPrism/Profiler/scripts/cann_profile_test_suite.py \
  --out /tmp/flagtree_profiler_cann_flaggems_ops_list \
  --clean \
  --with-flaggems \
  --list-flaggems-ops \
  --flaggems-all
...
- `mstx_ranges`：应包含 `flagtree_profiler_cann_flaggems::<case>` 和部分 Triton/CANN kernel range。
...
- `mstx_ranges` 包含 `flagtree_profiler_cann_flaggems::test_add`、`flagtree_profiler_cann_flaggems::test_addmm`、`flagtree_profiler_cann_flaggems::test_amax`。
