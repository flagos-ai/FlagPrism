...
FlagTree Profiler 是 FlagTree 的 Triton profiling 工具。它提供 session、scope、
Triton hook 和数据导出能力，并增加 IR instrumentation 路径，用来在不依赖特定
厂商 profiler 的情况下采集 kernel 内部 timeline。昇腾 CANN 的 legacy
`aclprof/msprof` 路径仍然保留，可通过环境变量显式打开。

Profiler 与 Debugger 由 `FlagPrism` submodule 维护，并随 FlagTree 主 wheel
共同构建和发布：
...
公开 Python API 为 `flagtree.profiler`，源码位于 `Profiler/`，native 模块为
`flagtree.profiler._native`。CLI 使用 `flagtree-profiler` 和
`flagtree-profiler-viewer`，运行时环境变量统一使用 `FLAGTREE_PROFILER_*`。
编译器内部的 `proton` dialect 名保持不变。

昇腾上默认的 `backend="cann", hook="triton"` IR 采集复用
`flagtree.debugger` 的插桩 runtime。Debugger 与 Profiler 只支持联合构建，统一由
`TRITON_BUILD_FLAGPRISM=ON` 开启；设为 `OFF` 时生成 core-only wheel。

当前用户接口仍然是 FlagTree Profiler API：
...
默认情况下，`backend="cann", hook="triton"` 会使用 FlagTree IR instrumentation，不会启动 CANN `aclprof/msprof`。`profile.hatchet` 可以直接用 `flagtree-profiler-viewer` 查看调用树和数值指标，例如：

```bash
flagtree-profiler-viewer -m flagtree.ir.duration_cycle,flagtree.ir.memory_access_bytes,flagtree.ir.estimated_bandwidth_bytes_per_cycle \
  /tmp/my_triton_profile/profile.hatchet
```
...
```bash
FLAGTREE_PROFILER_CANN_TRITON_HOOK_LEGACY=1 python3 your_program.py
flagtree-profiler-viewer -m time/us,cann.task_duration_us,cann.bandwidth_gb_s \
  /tmp/my_triton_profile/profile.hatchet
```
...
`FLAGTREE_PROFILER_CANN_TRITON_HOOK_LEGACY=1` 后，`hook="triton"` 才会恢复原来的 kernel
...
- `stream <id>` 线程：实际 kernel/CANN task 事件。Timeline 主视图只画真实
  设备侧 task，不再把 FlagTree Profiler scope/range 作为单独事件画出来。
...
`args.metrics` 和 `profile.vendor.json` 中。FlagTree Profiler scope 信息保留在每个事件的
...
`profile.hatchet` 保存适合树形聚合和 `flagtree-profiler-viewer` 展示的数值字段。
`flagtree-profiler-viewer -m` 可以传一个或多个字段名。时间、cycle、bytes、KB、
...
```bash
flagtree-profiler-viewer -m time/ms,cann.task_duration_us,cann.aicore_time_us,cann.bandwidth_gb_s,cann.memory_access_bytes \
  /tmp/my_triton_profile/profile.hatchet
```
...
- `time/ms`、`time/us`、`time/ns`：`flagtree-profiler-viewer` 展示的默认时间字段。有 CANN
  `op_summary` begin/finish 数据时，它表示设备侧 elapsed 时间：父节点取子树
  最早 task start 到最晚 task end，叶子节点表示该 task 的设备执行窗口，输出
  名会显示为 `time/us (cann elapsed)`。没有 CANN 数据时回退到 FlagTree Profiler 原生
  `time (ns)`。
- `time (ns)`：Hatchet 文件中保存的 FlagTree Profiler 原生时间，主要对应 host launch/range
  时间；CANN 后端分析设备耗时时通常不直接使用这个原始字段。
- `avg_time/ms`、`avg_time/us`、`avg_time/ns`：`time / count` 的平均耗时，由 `flagtree-profiler-viewer` 派生。
- `count`：当前节点累计的事件次数。
...
- `gbyte/s`、`tbyte/s`、`gflop/s`、`tflop/s`、`gflop16/s`、`tflop16/s`：`flagtree-profiler-viewer` 基于 `time` 和 `bytes` / `flops` / `flops16` 派生的吞吐指标。
- `cann.task_duration_us`：CANN/MSTX 关联到 FlagTree Profiler scope 后的任务持续时间，单位 us。通常用于看设备侧 kernel 耗时。
...
- `cann.tflop_s`、`cann.tflop8_s`、`cann.tflop16_s`、`cann.tflop32_s`、`cann.tflop64_s`：`flagtree-profiler-viewer` 结合 CANN `task_duration_us` 和 Triton workload hint 派生的有效计算吞吐。父节点会按 `sum(flops*) / sum(cann.task_duration_us)` 重新计算，不会把子节点吞吐率直接相加。
- `cann.bandwidth_gb_s`：CANN 导入的原始带宽，单位 GB/s。该字段不能跨父子节点直接相加；父节点通常显示 `NaN`。
- `cann.estimated_bandwidth_gb_s`：`flagtree-profiler-viewer` 结合 `bytes` 和 CANN `task_duration_us` 派生的有效带宽，单位 GB/s。父节点会按 `sum(bytes) / sum(cann.task_duration_us)` 重新计算。
...
`flagtree-profiler-viewer --list /tmp/my_triton_profile/profile.hatchet` 和
`profile.vendor.json` 为准。需要查看 CANN 原始字段和字符串元数据时，使用：

```bash
python3 third_party/FlagPrism/Profiler/scripts/cann_vendor_raw_report.py \
  /tmp/my_triton_profile/profile.vendor.json
```

### 参数说明

`profiler.start()` 的顶层参数：
...
- `context="shadow"`：使用 FlagTree Profiler scope/shadow context 组织调用树。配合 Triton hook 和手动 scope 时，输出更适合按算子或代码区域聚合。
...
- `runtime_base`：启用基础 runtime profiling。即使 vendor 数据不完整，也会保留 FlagTree Profiler 的基础运行时信息。
...
- `mstx_enabled=true`：启用 MSTX range 标注。开启后，FlagTree Profiler scope/Triton hook 产生的范围可以进入 CANN/MindStudio profiler 数据，并出现在 `vendor.json` 和 `timeline.json` 中。
- `mstx_domain=flagtree_profiler`：MSTX domain 名称，也是默认值，便于在导出的 CANN 数据中识别 Profiler range。

默认行为和调试开关：

- 昇腾上 `hook="triton"` 默认启用 IR instrumentation，不启动 CANN `aclprof/msprof`。需要恢复旧 CANN 路径时设置环境变量 `FLAGTREE_PROFILER_CANN_TRITON_HOOK_LEGACY=1`。
- `runtime_host_timing_fallback` 默认开启。当 CANN runtime event 不完整时，Profiler 会用 host timing 辅助关联 CANN op summary 和 FlagTree Profiler scope；普通用户不需要配置。需要关闭时设置环境变量 `FLAGTREE_PROFILER_CANN_RUNTIME_HOST_FALLBACK=0`。
- IR 自动插桩默认为每次 kernel launch 分配 32 MiB record buffer（`524288` 条、64 bytes/条），kernel 结束导出后释放。可通过 `FLAGTREE_PROFILER_IR_RECORD_BUFFER_MB=<MiB>` 调整；复杂 kernel 每个 program instance 包含更多 IR op，需要更大的 buffer 才能覆盖相同数量的 instance。
- `aclprof_output_path` 默认不需要设置。Profiler 会使用内部临时目录收集 CANN 原始数据，导入 `profile.vendor.json` 后自动清理。需要保留 CANN 原始 `PROF_*` 和 CSV 时，设置环境变量 `FLAGTREE_PROFILER_CANN_PROFILE_OUTPUT=/tmp/my_triton_profile/msprof`。
- `aclprof_runtime_enabled=false`：关闭 FlagTree Profiler 内部启动 CANN aclprof。适合只做已有 CSV 的 post-import 或外部 `msprof` 包裹验证。
...
- 旧 CANN legacy 路径通过 `FLAGTREE_PROFILER_CANN_TRITON_HOOK_LEGACY=1` 启用。该路径支持基础 runtime profiling 和 CANN vendor metrics 导入，包括 AICore、MSTX range、bandwidth 相关指标。
- legacy 路径的 `finalize()` 内部会自动调用 CANN 的 `msprof --export=on` 把 profiler 原始目录导出成 CSV，然后导入到 FlagTree Profiler artifact。用户不需要手动用 `msprof python ...` 包住程序。
- Triton kernel 始终通过 `hook="triton"` 自动进入 FlagTree Profiler scope。
...
一个带 FlagTree Profiler scope 的多算子 Tiny MLP，适合在介绍 GPT 示例前解释多 kernel
调用树和父子节点聚合：
...
- `examples/tiny_mlp_profile.py`：两层 Tiny MLP，包含 linear、ReLU、linear 三个 Triton kernel，并用 `profiler.scope` 组织 `tiny_mlp/1_layer1/2_activation/3_layer2` 调用树。
...
5. **FlagTree Profiler 生命周期层**：`Session` 在 `start()` 时创建 profiler，在 `finalize()` 时停止采集、触发导出和基础 artifact 写入。CANN legacy 路径仍然通过这里创建 vendor profiler。相关文件：`third_party/FlagPrism/Profiler/csrc/lib/Session/Session.cpp`。

6. **Vendor adapter 层**：`Adapter` 根据 `backend` 创建具体厂商后端，并解析通用 `mode` 配置。它是可选增强层，不是默认 IR profiler 的必要条件。相关文件：`csrc/include/Profiler/Vendor/Adapter.h`、`csrc/lib/Profiler/Vendor/Adapter.cpp`、`Mode.cpp`。

7. **CANN legacy 后端层**：`CannProfiler` 负责调用/控制 CANN profiling，处理 MSTX range、自动 `msprof --export=on`、CSV 导入，以及把 AICore、bandwidth、runtime/API 等数据关联到 FlagTree Profiler scope。相关文件：`csrc/lib/Profiler/Vendor/CannProfiler.cpp`、`CannAdapter.cpp`、`Driver/Ascend/AscendApi.cpp`。

Profiler wheel 的 Python、native runtime、CLI、生命周期和 vendor adapter 均位于
`third_party/FlagPrism/Profiler`；FlagTree 主体只保留组件门面和稳定接入点。详见
[目录结构](directory_structure.md)。

## 代码位置

Profiler 自身代码集中在 `third_party/FlagPrism/Profiler/`：
...
- `csrc/include/Profiler/Vendor/CannProfiler.h`、`csrc/lib/Profiler/Vendor/CannProfiler.cpp`：CANN runtime profiling、MSTX range、aclprof/msprof 导出、CSV 导入、CANN metric 与 FlagTree Profiler scope 关联。
...
FlagTree Profiler 的公共接入点位于：

- `python/flagtree_profiler/profile.py`：Python API，把 `backend`、`hook`、`mode` 传到 C++ session。
- `python/flagtree_profiler/hooks/hook.py`：`hook="triton"` 的 Python launch hook。
- `python/flagtree_profiler/viewer.py`：`flagtree-profiler-viewer`，负责 Hatchet 树展示和字段聚合/派生规则。
- `csrc/include/Profiler/Profiler.h`：FlagTree Profiler profiler 生命周期基类，定义 `doStart()`、`doStop()`、`doSetMode()`。
- `csrc/include/Session/Session.h`、`csrc/lib/Session/Session.cpp`：`profiler.start()` / `profiler.finalize()` 生命周期，创建 vendor profiler，停止采集并写出 artifact。
...
- `CMakeLists.txt`：把 `csrc` 编译为 `flagtree.profiler._native`。
...
- `third_party/FlagPrism/Debugger/lib/Instrumentation/Passes.cpp`：把 debugger record op lowering 成设备侧 ring buffer 写入。
- `third_party/FlagPrism/Debugger/python/flagtree_debugger/api.py`、`runtime.py`：准备 hidden arg、导出和 decode runtime records。
...
新增芯片后端有两条路径。默认建议先接 IR profiler，因为它能复用 FlagTree Profiler API 和 artifact，不要求目标后端提供 `msprof` 这类厂商工具。
...
4. **artifact 合成复用**：只要导出的 runtime records 符合 FlagPrism debugger decode 结构，`third_party/FlagPrism/Profiler/python/flagtree_profiler/profile.py` 中的 artifact 合成逻辑可以直接复用，生成 `flagtree.ir.*` 指标和四个输出文件。
...
2. **`Profiler`**，接入 FlagTree Profiler 的 start/finalize 生命周期，负责启动和停止厂商 runtime profiler。

   需要实现的接口在 `third_party/FlagPrism/Profiler/csrc/include/Profiler/Profiler.h`：
...
3. **`VendorMetricsImporter`**，把厂商 profiler 的导出文件或内存结果转换成 FlagTree Profiler 统一 artifact。
...
```python
profiler.start(..., backend="<vendor>", hook="triton", mode="...")
profiler.finalize(sid)
```
