# FlagTree Profiler CANN Liger-Kernel Full Suite

This suite profiles a real open-source Triton operator library:

https://github.com/linkedin/Liger-Kernel

It validates that FlagTree Profiler CANN can profile Triton kernels coming from a public
...
python third_party/FlagPrism/Profiler/scripts/cann_profile_test_suite.py \
  --out /tmp/flagtree_profiler_cann_liger_full \
  --with-liger \
  --clean
```
...
The script uses FlagTree Profiler's direct workflow:

```python
sid = profiler.start(..., backend="cann", hook="triton", ...)
# run Liger operators
